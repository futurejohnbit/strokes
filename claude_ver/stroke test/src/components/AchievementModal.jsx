...
const AchievementModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-4 sm:p-5">
      <div className={`bg-white rounded-[2.25rem] shadow-2xl w-full max-w-xl max-h-[calc(100vh-2rem)] mx-auto overflow-y-auto border-4 transform transition-all scale-100 ${tone.shell}`}>
        
        {/* Header: Title & Icon */}
        <div className={`px-6 py-6 md:px-8 md:py-7 text-center border-b-2 ${tone.header}`}>
          <div className={`inline-flex items-center justify-center w-[4.5rem] h-[4.5rem] md:w-20 md:h-20 rounded-full mb-4 shadow-inner animate-bounce ${tone.icon}`}>
            {profession?.icon || <Award size={48} />}
          </div>
          <div className={`inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] ${tone.pill}`}>
            <Sparkles size={14} />
            {rewardAccent}
          </div>
          <h2 className="mt-4 text-3xl md:text-[2rem] font-bold text-slate-800 font-kai leading-tight">
            {title}
          </h2>
          <p className="text-slate-500 mt-3 font-medium leading-7">{subtitle}</p>
        </div>

        {/* Content: Stats */}
        <div className="px-5 py-6 md:px-8 md:py-8 space-y-6 md:space-y-7">
          
          {/* Badge Display */}
          <div className="flex justify-center">
            <div className="w-32 h-32 sm:w-36 sm:h-36 md:w-40 md:h-40 bg-white rounded-full border-4 border-slate-200 flex items-center justify-center relative shadow-lg group hover:border-amber-400 transition-colors">
          ...
          <div className={`rounded-[1.75rem] border px-5 py-5 md:px-6 md:py-6 ${tone.reward}`}>
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
              <div className={`rounded-full p-3 shadow-sm w-fit ${tone.icon}`}>
                <Sparkles size={18} />
              </div>
              <div className="min-w-0 flex-1">
                <div className="text-xs font-bold uppercase tracking-[0.2em] text-slate-400">本次收成</div>
                <div className="mt-2 text-[1.75rem] leading-tight font-bold text-slate-800">{rewardLabel}</div>
                <div className="mt-3 text-sm leading-7 text-slate-500">{rewardSummary}</div>
              </div>
            </div>
          </div>
        ...
        {/* Footer: Actions */}
        <div className="px-5 py-5 md:px-8 md:py-6 bg-slate-50 border-t border-slate-100 flex flex-col gap-3 sm:flex-row sm:gap-4">
          <button 
            onClick={onMenu}
