...
const DIRECT_STROKE_TOKENS = new Set([
  'HENG',
  'SHU',
  'PIE',
  'NA',
...
export default function StrokeTest() {
  ...
  const labelFromToken = (token) => {
    ...
    if (token === 'TI') return '⬈ 提 (㇀)';
    if (token === 'HENGPIE') return '⤴️ 橫撇';
    if (token === 'HENGZHE') return '⤵️ 橫折 (ㄱ)';
    if (token === 'HENGZHEGOU') return '🪝 橫折勾';
    if (token === 'HENGSHUGOU') return '🪝 橫豎鉤';
    if (token === 'SHUGOU') return '🪝 豎鉤 (亅)';
    return '未知';
  };

  const normalizeStrokeToken = useCallback((token) => STROKE_CODE_TO_TOKEN[token] || token, []);

  const applyDirectStrokeToken = useCallback((token, sourceLabel) => {
    const label = labelFromToken(token);
    mlHintRef.current = { token, ts: Date.now() };
    lastStrokeRef.current = { ...lastStrokeRef.current, label };
    setCurrentStroke(label);
    addLog(`收到筆劃碼(${sourceLabel}): ${token} -> ${label}`);
    drawCanvas();
  }, [drawCanvas]);
  ...
  const handleData = useCallback((event) => {
    ...
    for (const token of tokens) {
      ...
      //#region debug-point stroke-test-single-byte-ble
      if (/^([1-9]|10)$/.test(token)) {
        addLog(`偵測到單字節筆劃碼(BLE): ${token}`);
      }
      //#endregion

      const up = token.toUpperCase();
      const normalizedToken = normalizeStrokeToken(up);
      if (DIRECT_STROKE_TOKENS.has(normalizedToken)) {
        applyDirectStrokeToken(normalizedToken, 'BLE');
        continue;
      }
      ...
      if (token.startsWith('{') && token.endsWith('}')) {
        try {
          ...
          if (obj?.kind === 'imuAssist') {
            addLog(`IMU assist(BLE): dir=${obj.imuDir || 'unknown'} score=${obj.pieScore || 0} for=${obj.forCode || '-'}`);
            continue;
          }
  ...
  const handleSerialChunk = useCallback((text) => {
    ...
    for (const raw of tokens) {
      ...
      if (!token) continue;
      if (/^VI_\d+$/i.test(token) || /^ID_PROP$/i.test(token)) continue;
      //#region debug-point stroke-test-single-byte-serial
      if (/^([1-9]|10)$/.test(token)) {
        addLog(`偵測到單字節筆劃碼(Serial): ${token}`);
      }
      //#endregion
      const up = token.toUpperCase();
      const normalizedToken = normalizeStrokeToken(up);
      if (DIRECT_STROKE_TOKENS.has(normalizedToken)) {
        applyDirectStrokeToken(normalizedToken, 'Serial');
        continue;
      }
      if (token.startsWith('{') && token.endsWith('}')) {
        try {
          const obj = JSON.parse(token);
          if (obj?.kind === 'imuAssist') {
            addLog(`IMU assist(Serial): dir=${obj.imuDir || 'unknown'} score=${obj.pieScore || 0} for=${obj.forCode || '-'}`);
            continue;
          }
          const x = typeof obj.x === 'number' ? obj.x : (obj.pos && typeof obj.pos.x === 'number' ? obj.pos.x : null);
          const y = typeof obj.y === 'number' ? obj.y : (obj.pos && typeof obj.pos.y === 'number' ? obj.pos.y : null);
  ...
  }, [applyDirectStrokeToken, finalizeStroke, normalizeStrokeToken, processPoint, wsSend]);
  ...
  const connectWs = async () => {
    try {
      ...
      if (typeof navigator !== 'undefined' && navigator.onLine === false) {
        throw new Error('目前處於離線狀態，無法建立 WebSocket 連線');
      }
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white p-8 flex flex-col items-center">
      ...
      <div className="mt-8 text-slate-400 text-sm max-w-2xl text-center">
        支援 "X,Y" / "X,Y,Z"、JSON {"{\"x\":...,\"y\":...}"}、單字節筆劃碼 "1-10"；也可接收 Gate 控制行 "G,1" / "G,0"、ML token（HENG/SHU/PIE/NA/TI...）與 `imuAssist` JSON。
      </div>
    </div>
  );
}
