...
## 正式頁撇補強
- 正式遊戲頁檔案：[GeminiApp.jsx](file:///c:/Expoprojects/microtesting/claude_ver/stroke%20test/src/GeminiApp.jsx)
- 輔助規則模組：[imuPieAssist.js](file:///c:/Expoprojects/microtesting/claude_ver/stroke%20test/src/utils/imuPieAssist.js)
- micro:bit 事件發送範例：[microbit-updated-code.py](file:///c:/Expoprojects/microtesting/claude_ver/stroke%20test/microbit-updated-code.py)
- 第一版只補強 `撇 / left-down`
- `CreateAI` 仍是主判斷來源
- 只有在目標筆畫是 `left-down`、且 `CreateAI` 沒判成 `left-down` 時，前端才會短暫等待 `MPU6050` 的輔助訊號
- 若 `MPU6050` 在短時間窗口內回傳高信心 `left-down`，前端會把該次輸入升級為 `left-down`
- 若 `CreateAI` 本來就判對，`MPU6050` 不會否決它

### BLE side-channel 協定
- 原本單字節筆畫碼維持不變，例如：
  - `1`
  - `6`
- 額外新增一條 `imuAssist` JSON，例如：

```json
{"kind":"imuAssist","forCode":"6","imuDir":"left-down","pieScore":0.84,"sumGX":-126,"sumGY":92,"sampleCount":12}
```

- 欄位說明：
  - `kind`: 固定為 `imuAssist`
  - `forCode`: 這條輔助訊號對應的 CreateAI 筆畫碼
  - `imuDir`: 6050 判出的方向，目前正式頁只關心 `left-down`
  - `pieScore`: `0~1` 的撇信心分數
  - `sumGX` / `sumGY`: 短時間窗口內的角速度累積值，主要用於 debug 與校門檻
  - `sampleCount`: 此次計算用到的樣本數
...
- 用 `/debug` 收集至少：
  - 10 次撇
  - 10 次非撇，至少包含捺
- 觀察 `sumGX`、`sumGY`、`pieScore`
- 再回正式頁檢查：
  - `CreateAI` 本來判對的撇不受影響
  - `CreateAI` 誤判時，6050 能補強部分撇
  - 非撇不會被大量誤升級
